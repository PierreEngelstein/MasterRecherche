...
p1prime = p1 * np.dot(psi_1, np.transpose(psi_1))
p2prime = p2 * np.dot(psi_2, np.transpose(psi_2))
p3prime = p3 * np.dot(psi_3, np.transpose(psi_3))

# Formulate the problem and constraints, and solve
X = cp.Variable((2, 2), symmetric=True)
constraints =  [X >> p1prime]
constraints += [X >> p2prime]
constraints += [X >> p3prime]
...
# Y = np.hstack((q1_v, q2_v, q3_v, [[0], [1], [0], [0]]))
...
print(pi1 + pi2 + pi3)
# (X_sol - pi')PI_i = 0
print(np.dot(X_sol - p1prime, pi1))
print(np.dot(X_sol - p2prime, pi2))
print(np.dot(X_sol - p3prime, pi3))